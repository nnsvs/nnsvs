...
from nnsvs.io.hts import (full_to_mono, get_note_indices, label2phrases,
                          label2phrases_str, merge_sil, segment_labels)


def test_full_to_mono():
    full_labels = hts.load(join(dirname(__file__), "data", "neutrino", "sample1_full.lab"))
    mono_labels = hts.load(join(dirname(__file__), "data", "neutrino", "sample1_mono.lab"))
...
def test_get_note_indices(filename):
    mono_labels = hts.load(join(dirname(__file__), "data", "neutrino", f"{filename}_mono.lab"))
    note_indices1 = get_note_indices(mono_labels)

    full_labels = hts.load(join(dirname(__file__), "data", "neutrino", f"{filename}_full.lab"))
...
def test_label2phrases_str(filename):
    ...
    timing_labels = hts.load(join(dirname(__file__), "data", "neutrino", f"{filename}_timing.lab"))
    nnsvs_str = label2phrases_str(timing_labels, note_indices)

    with open(join(dirname(__file__), "data", "neutrino", f"{filename}-phraselist.txt"), "r") as f:
        neutrino_str = f.read()
