...
class TestMDN(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.model=MDN(self.d_in, hidden_dim, self.d_out, num_layers=num_layers, num_gaussians=num_gaussians).to(self.device)
    ...
    def test_mdn_loss(self):
        ...
        x = torch.from_numpy(self.x_train_inv.reshape(self.batch_size, -1, self.d_in)).to(self.device) # (B, max(T), D_in)
